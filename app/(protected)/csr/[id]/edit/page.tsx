// app/(protected)/csr/[id]/edit/page.tsx
'use client';
...
export default function EditCSRPage({ params }: Params) {
        ...
        fetch(`/api/csr/${id}`)
            ...
            .then(data => {
                ...
                setForm({
                    title: data.title || '',
                    slug: data.slug || '',
                    category: data.category || 'Education & Welfare',
                    date: data.date || '',
    ...
    const updateExistingItem = (index: number, patch: Partial<Asset>) => {
        setExistingItems(prev => prev.map((item, i) => i === index ? { ...item, ...patch } : item));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errData.error || 'Failed to update CSR post');
            }
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
                ...
                <p className="text-sm text-slate-400 font-medium">Loading post details…</p>
            </div>
        );
    }
        ...
        <div className="min-h-[calc(100vh-64px)] bg-[#f9fbfd] pb-20">
            ...
            <div className="bg-white border-b border-slate-100 px-4 lg:px-8 py-5">
                <div className="flex items-center gap-1.5 mb-3 text-xs text-slate-400 font-medium">
                    ...
                    <span className="text-slate-700 font-semibold truncate max-w-[200px]">Edit Post</span>
                </div>

                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-bold text-slate-900 leading-tight">Edit CSR Post</h1>
                        <p className="text-sm text-slate-400 mt-0.5">Modify community impact campaign or initiative details.</p>
                    </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="bg-white p-6 border border-slate-100 shadow-sm rounded-sm space-y-6">
                        ...
                        <div className="space-y-4 pt-4 border-t border-slate-100">
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Asset Queue ({existingItems.length + newItems.length})</div>
                            ...
                            {existingItems.length === 0 && newItems.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {/* Render Existing Database Items */}
                                    {existingItems.map((item, idx) => (
                                        <div key={`existing-${idx}`} className="border border-slate-100 rounded-xl p-4 bg-indigo-50/30 relative space-y-3 flex flex-col justify-between">
                                            <button 
                                                type="button"
                                                ...
                                                <div className="space-y-2 pt-1">
                                                    <div className="space-y-1">
                                                        ...
                                                        <input 
                                                            type="text"
                                                            value={item.title}
                                                            onChange={e => updateExistingItem(idx, { title: e.target.value })}
                                                    ...
                                                    <div className="space-y-1">
                                                        ...
                                                        <input 
                                                            ...
                                                            onChange={e => updateExistingItem(idx, { description: e.target.value })}
                                    ...
                                    {/* Render New Upload Queue Items */}
                                    {newItems.map((item, idx) => (
                                        <div key={`new-${idx}`} className="border border-slate-100 rounded-xl p-4 bg-emerald-50/20 relative space-y-3 flex flex-col justify-between">
                                            <button 
                                                type="button"
