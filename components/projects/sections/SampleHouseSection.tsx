// components/projects/sections/SampleHouseSection.tsx
'use client';
import { useRef, useState, DragEvent } from 'react';
import { CloudUpload, X, Loader2, Trash2, RotateCcw, Plus, ArrowUpCircle, Camera } from 'lucide-react';
import { uploadSamplePhotos, updateSamplePhotos } from '@/lib/projectApi';
import type { ProjectDoc, SampleHousePhoto, SampleHousePreview } from '@/types/project';

interface Props { project: ProjectDoc; onUpdate: (doc: ProjectDoc) => void; }

const ROOM_TYPES = ['Living Room', 'Master Bedroom', 'Bedroom', 'Kitchen', 'Bathroom', 'Balcony', 'Dining Area', 'Study Room', 'Other'];

export default function SampleHouseSection({ project, onUpdate }: Props) {
    const inputRef = useRef<HTMLInputElement>(null);
    const [dragging, setDragging] = useState(false);
    const [previews, setPreviews] = useState<SampleHousePreview[]>([]);
    ...
    const addFiles = (list: FileList | File[]) => {
        ...
        setPreviews(prev => [
            ...
            ...files.map((file, i) => ({
                ...
                alt: '', roomType: '', order: prev.length + i,
            })),
        ]);
    };
    ...
    const updatePreview = (i: number, patch: Partial<SampleHousePreview>) =>
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (project.sampleHousePhotos.length > 0 || deletions.size > 0) {
                const existingDetails: (SampleHousePhoto & { markedForDeletion?: boolean })[] =
                    project.sampleHousePhotos.map(p => ({ ...p, markedForDeletion: p._id ? deletions.has(p._id) : false }));
                updated = await updateSamplePhotos(project._id, previews.map(p => p.file), [...existingDetails, ...previews]);
            } else {
                updated = await uploadSamplePhotos(project._id, previews.map(p => p.file), previews);
            }
            onUpdate(updated);
    ...
    const existing = project.sampleHousePhotos ?? [];

    return (
        <div className="space-y-5 max-w-3xl">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <h2 className="text-base font-bold text-slate-800 mb-1">Sample House Photos</h2>
                <p className="text-xs text-slate-400 mb-4">Interior photos of the sample/model flat — tag each with a room type</p>
                ...
                >
                    <div className="flex flex-col items-center py-10 gap-2">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${dragging ? 'bg-indigo-500' : 'bg-slate-50 border border-slate-200'}`}>
                            <Camera size={20} className={dragging ? 'text-white' : 'text-slate-400'} />
                        </div>
                        <p className="text-sm font-semibold text-slate-600">{dragging ? 'Drop photos' : 'Upload sample house photos'}</p>
                    </div>
            ...
            {existing.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                        <p className="text-sm font-semibold text-slate-800">Current Photos ({existing.length})</p>
                        {deletions.size > 0 && (
                        ...
                        {existing.map((photo) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <img src={photo.url} alt={photo.alt ?? photo.title} className="w-full aspect-video object-cover" />
                                    {photo.roomType && (
                                        <div className="absolute bottom-1.5 left-1.5 bg-black/50 backdrop-blur-sm text-white text-[9px] font-semibold px-2 py-0.5 rounded">
                                            {photo.roomType}
                                        </div>
                                    )}
                                    <button
                                        type="button"
                                        onClick={() => photo._id && toggleDeletion(photo._id)}
            ...
            {previews.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                        <p className="text-sm font-semibold text-slate-800">New Photos ({previews.length})</p>
                        ...
                        {previews.map((p, i) => (
                            <div key={i} className="border border-slate-100 rounded-xl overflow-hidden">
                                ...
                                <div className="p-3 space-y-2">
                                    ...
                                    <select value={p.roomType} onChange={e => updatePreview(i, { roomType: e.target.value })}
            ...
            <div className="flex justify-end">
                ...
                >
                    {uploading ? <><Loader2 size={14} className="animate-spin" /> Uploading…</> : <><ArrowUpCircle size={14} /> Save Photos</>}
                </button>
            </div>
        </div>
    );
