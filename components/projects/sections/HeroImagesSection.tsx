        ...
        try {
            ...
            if (project.heroImages.length > 0 || deletions.size > 0) {
                const existingDetails: (MediaItem & { markedForDeletion?: boolean })[] = project.heroImages.map(img => ({
                    ...img, markedForDeletion: img._id ? deletions.has(img._id) : false,
                }));
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5 p-4">
                        {existing.map((media) => {
                            ...
                            return (
                                <div key={media._id}
                                    className={`group relative rounded-xl overflow-hidden aspect-video transition-all
                    ${isDeleted ? 'opacity-40 ring-2 ring-rose-400' : ''}`}
                                >
                                    ...
                                    {media.isMainImage && (
                                        <span className="absolute top-1 left-1 bg-amber-400 text-white text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5">
